...
- Matthew J. Heaton ([https://statistics.byu.edu/directory/heaton-matthew](https://statistics.byu.edu/directory/heaton-matthew))

- Dan S. Bolintineanu (Sandia National Laboratories)

- Aaron Olson (Sandia National Laboratories)

## Introduction

The primary challenge of spatial statistics is to account for the inherent correlation that arises in the data due to proximity in sampling locations. To account for this correlation, spatial statisticians traditionally use the Gaussian process (GP). However, GPs are plagued by computational intractability, rendering them infeasible for use on large spatial data sets. On the other hand, neural networks (NNs) have arisen as a flexible approach for modeling nonlinear relationships. To date, however, NNs have only been scarcely used for problems in spatial statistics. In this research work, we implement NNs for spatial data (for a writeup of the work we have done as of April 15, 2022, see [**Substituting Neural Networks for Gaussian Processes**](https://github.com/skylerg022/nn-as-gp/blob/main/masters_writeup_4_15.pdf)). This directory contains the code used to fit and predict with NNs from this research.
...
**Disclaimer**: These results are not entirely replicable because of parallelization and large search grids used in optimizing hyperparameters.
...
Only the data used in **ToyDataset** and **Binary1Million** are included in this repository. A brief description and the data file locations of each dataset are provided below (Note: you will need to create the **data** directory within each dataset folder):
